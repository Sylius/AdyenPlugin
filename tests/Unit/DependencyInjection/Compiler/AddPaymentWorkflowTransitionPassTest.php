...
final class AddPaymentWorkflowTransitionPassTest extends TestCase
{
    ...
    {
        $this->compilerPass = new AddPaymentWorkflowTransitionPass();
    ...
    public function testItDoesNothingWhenWorkflowDefinitionDoesNotExist(): void
    {
        ...
        $this->assertFalse($this->container->hasDefinition('state_machine.sylius_payment.definition'));
    }
    ...
    public function testItAddsRequiredTransitionsWhenWorkflowDefinitionExists(): void
    {
        ...
        $this->container->setDefinition('state_machine.sylius_payment.definition', $workflowDefinition);

        $this->compilerPass->process($this->container);

        $definition = $this->container->getDefinition('state_machine.sylius_payment.definition');
        $transitions = $definition->getArgument(1);

        $this->assertCount(4, $transitions);

        $this->assertTransitionExists($transitions, 'process', ['authorized'], ['processing']);
        $this->assertTransitionExists($transitions, 'fail', ['completed'], ['failed']);
        $this->assertTransitionExists($transitions, 'cancel', ['processing_reversal'], ['cancelled']);
        $this->assertTransitionExists($transitions, 'refund', ['processing_reversal'], ['refunded']);
    }

    public function testItDoesNotAddDuplicateTransitions(): void
    {
        $existingTransition = new Definition(Transition::class);
        $existingTransition->setArguments(['process', ['authorized'], ['processing']]);
        ...
        $this->assertSame(1, $processTransitionsCount);
    }
